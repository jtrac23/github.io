...
//some jQuery stuff
$("#playground").playground({heaight: PLAYGROUND_HEIGHT, width: PLAYGROUND_WIDTH, keyTracker: true});
...
$.playground().registerCallback(function(){
  ...
  $("#player").each(function(){
		//var score = $("#player")[0].player.value;
		if($(this)[0].player.value >= 100){
		  $.playground().pauseGame(true);
		  //$.playground().clearAll(true);
			$("#messages").html("<h1><center>You Win!!!</center></h1>");	 
		};
		if($(this)[0].player.value <= -100){
			$.playground().pauseGame(true);
			$("#messages").html("<h1><center>You Lose!</center><h1>");
		};
  });
...
$.playground().registerCallback(function(){
	...
	enemyElement[0].enemy = new Enemy(enemyElement, enemyValue); //one would assume these are the node and value
	enemyElement.text(enemyValue);
}, enemySpawnRate);

//fire them missileSpeed
...
/*
TODO
Make a messages DIV that displays YOU WIN/YOU LOSE
stop the game when 100 or -100 is reached win/lose respectively
